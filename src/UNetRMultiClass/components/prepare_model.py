...
class PrepareModel:
    ...
    def transformer_encoder(self, x):
        ...
        x = L.MultiHeadAttention(
            ...
            key_dim=self.config.params_hidden_dim,
        )(x, x)
        x = L.Add()([x, skip_1])
    ...
    def get_full_model(self):
        ...
        input_shape = (
            self.config.params_num_patches,
            self.config.params_patch_size
            * self.config.params_patch_size
            * self.config.params_num_channels,
        )
        inputs = L.Input(input_shape)  ## (None, 256, 768)

        """ Patch + Positional Embeddings """
        patch_embed = L.Dense(self.config.params_hidden_dim)(
            inputs
        )  ## (None, 256, 768)

        positions = tf.range(
            start=0, limit=self.config.params_num_patches, delta=1
        )  ## (256, )

        pos_embed = L.Embedding(
            input_dim=self.config.params_num_patches,
            output_dim=self.config.params_hidden_dim,
        )(
            positions
        )  ## (256, 768)
        ...
        outputs = L.Conv2D(
            ...
            x
        )  ## 1 -> binary segmentation and hence the sigmoid fxn, can change for multi-class
        full_model = Model(inputs, outputs, name="UNETR_2D")
        full_model.compile(
            loss="categorical_crossentropy",
            optimizer=SGD(self.config.params_learning_rate),
        )
