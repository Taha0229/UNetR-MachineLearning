...
class Training:
    ...
    def get_base_model(self):
        self.model = tf.keras.models.load_model(self.config.untrained_model_path)
    ...
    def read_image(self, path):
        ...
        image = cv2.resize(
            image, (self.config.params_image_size, self.config.params_image_size)
        )
        image = image / 255.0
    ...
    def read_mask(self, path):
        ...
        mask = cv2.resize(
            mask, (self.config.params_image_size, self.config.params_image_size)
        )
    ...
    def train(self, callback_list: list):
        ...
        train_dataset = self.tf_dataset(
            train_x, train_y, batch=self.config.params_batch_size
        )
        valid_dataset = self.tf_dataset(
            valid_x, valid_y, batch=self.config.params_batch_size
        )
